...
@section('title', 'Literacy Content Management')

@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Literacy Content</h1>
            <p class="mt-1 text-gray-600">Manage educational resources for students</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
        <form method="GET" action="{{ route('admin.literacy-contents.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <!-- Type Filter -->
                <div>
                    <label for="type" class="block text-sm font-medium text-gray-700 mb-1.5">Content Type</label>
                    <select name="type"
                            id="type"
                        ...
                        <option value="article" {{ $type === 'article' ? 'selected' : '' }}>Articles</option>
            ...
            <div class="flex flex-wrap items-center gap-3 pt-2">
                ...
                <span class="text-sm text-gray-500 ml-auto">
                    Total: <strong>{{ $typeCounts['all'] }}</strong> content items
                </span>
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Platform</th>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            ...
                            <h3 class="text-lg font-semibold text-gray-800 mb-2">No literacy content found</h3>
                            <p class="text-gray-500">
                                @if($search)
                                    Try adjusting your search terms.
                                @else
                                    Get started by creating your first literacy content item or uploading multiple items via bulk upload.
                                @endif
                            </p>
            ...
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p class="text-sm text-gray-600">
                        Showing <strong>{{ $contents->firstItem() ?? 0 }}</strong> to <strong>{{ $contents->lastItem() ?? 0 }}</strong> of <strong>{{ $contents->total() }}</strong> content items
                    </p>
                    <div class="flex items-center gap-2">
