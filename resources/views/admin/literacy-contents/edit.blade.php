...
@section('title', 'Edit Literacy Content')
...
@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Edit Content</h1>
                <p class="mt-1 text-gray-600">Update educational resources for students</p>
            </div>
        </div>
    </div>
@endsection
...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="p-6">
                <form method="POST" action="{{ route('admin.literacy-contents.update', $literacyContent->id) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <!-- Type and Category -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="type"
                                ...
                                <option value="article" {{ $literacyContent->type === 'article' ? 'selected' : '' }}>Article</option>
                                <option value="video" {{ $literacyContent->type === 'video' ? 'selected' : '' }}>Video</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text"
                                   ...
                                   id="category"
                                   required
                                   value="{{ old('category', $literacyContent->category) }}"
                    ...
                    <div>
                        ...
                        <input type="text"
                               ...
                               required
                               maxlength="255"
                               value="{{ old('title', $literacyContent->title) }}"
                               class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200 text-gray-900 shadow-sm"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea name="content"
                                      ...
                                      placeholder="Enter article content (if applicable)">{{ old('content', $literacyContent->content) }}</textarea>
                        ...
                        <div>
                            ...
                            <input type="url"
                                   name="url"
                                   id="url"
                                   value="{{ old('url', $literacyContent->url) }}"
                                   class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200 text-gray-900 shadow-sm"
                    ...
                    <div>
                        ...
                        <input type="url"
                               name="thumbnail"
                               id="thumbnail"
                               value="{{ old('thumbnail', $literacyContent->thumbnail) }}"
                               class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200 text-gray-900 shadow-sm"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="platform"
                                ...
                                <option value="youtube" {{ $literacyContent->platform === 'youtube' ? 'selected' : '' }}>YouTube</option>
                                <option value="vimeo" {{ $literacyContent->platform === 'vimeo' ? 'selected' : '' }}>Vimeo</option>
                                <option value="coursera" {{ $literacyContent->platform === 'coursera' ? 'selected' : '' }}>Coursera</option>
                                <option value="udemy" {{ $literacyContent->platform === 'udemy' ? 'selected' : '' }}>Udemy</option>
                                <option value="medium" {{ $literacyContent->platform === 'medium' ? 'selected' : '' }}>Medium</option>
                                <option value="blog" {{ $literacyContent->platform === 'blog' ? 'selected' : '' }}>Blog</option>
                                <option value="other" {{ $literacyContent->platform === 'other' ? 'selected' : '' }}>Other</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="platform_id"
                                   id="platform_id"
                                   value="{{ old('platform_id', $literacyContent->platform_id) }}"
                                   class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200 text-gray-900 shadow-sm"
