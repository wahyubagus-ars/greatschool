...
@section('title', __('Redemption Management'))

@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">{{ __('Redemption Management') }}</h1>
            <p class="mt-1 text-gray-600">{{ __('Monitor and process student point redemptions') }}</p>
        </div>
...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-500">{{ __('Redeemed Today') }}</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-500">{{ __('Value Redeemed') }}</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-500">{{ __('Pending QR Codes') }}</p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <h2 class="text-lg font-semibold text-gray-800">{{ __('Recent Redemptions') }}</h2>
        ...
        @if($redemptions->isEmpty())
            <div class="p-8 text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-800 mb-2">{{ __('No redemptions found') }}</h3>
                <p class="text-gray-500 mb-6">{{ __('Student redemptions will appear here once generated.') }}</p>
                <a href="{{ route('admin.redemptions.scan') }}"
        ...
        @else
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                        <th scope="col" class="px-6 py-4 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">{{ __('Student') }}</th>
                    ...
                    @foreach($redemptions as $redemption)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                ...
                                @else
                                    <span class="text-gray-400">{{ __(ucfirst($redemption->status)) }}</span>
                                @endif
                            </td>
                        </tr>
                    @endforeach
