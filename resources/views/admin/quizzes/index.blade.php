...
@section('title', __('Quiz Management'))

@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">{{ __('Quizzes') }}</h1>
            <p class="mt-1 text-gray-600">{{ __('Manage educational quizzes for students') }}</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tr>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{{ __('Quiz') }}</th>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{{ __('Questions') }}</th>
                    ...
                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700">
                                {{ $quiz->points_per_quiz }} {{ __('points') }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                ...
                                <form action="{{ route('admin.quizzes.destroy', $quiz->id) }}" method="POST" class="inline">
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            onclick="return confirm('{{ __('Are you sure you want to delete this quiz?') }}')"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            ...
                            <h3 class="text-lg font-semibold text-gray-800 mb-2">{{ __('No quizzes found') }}</h3>
                            <p class="text-gray-500">{{ __('Get started by creating your first quiz or uploading multiple quizzes via bulk upload.') }}</p>
            ...
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p class="text-sm text-gray-600">
                        {{ __('Showing') }} <strong>{{ $quizzes->firstItem() ?? 0 }}</strong> {{ __('to') }} <strong>{{ $quizzes->lastItem() ?? 0 }}</strong> {{ __('of') }} <strong>{{ $quizzes->total() }}</strong> {{ __('results') }}
                    </p>
                    <div class="flex items-center gap-2">
