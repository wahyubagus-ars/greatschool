...
@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Report #{{ str_pad($facilityReport->id, 5, '0', STR_PAD_LEFT) }}</h1>
                <p class="mt-1 text-gray-600">{{ $facilityReport->title }}</p>
            </div>
        </div>
        <div>
            @php
            ...
            @endphp
            <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium border {{ $statusStyles[$facilityReport->status] }}">
                @if($facilityReport->status === 'pending')
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mr-2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                    </svg>
                @elseif($facilityReport->status === 'verified')
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mr-2">
                ...
                <div class="p-6 space-y-5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900">{{ $facilityReport->location }}</p>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Reported On</label>
                            <p class="text-sm font-medium text-gray-900">{{ $facilityReport->created_at->format('F d, Y \a\t H:i') }}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                            <p class="text-sm text-gray-800 whitespace-pre-line">{{ $facilityReport->description }}</p>
                        </div>
                    </div>
                    ...
                    @if($facilityReport->verification_note)
                        <div>
                            <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">
                                ...
                                @if($facilityReport->verifiedByAdmin)
                                    <span class="text-gray-400 normal-case">— by {{ $facilityReport->verifiedByAdmin->full_name }}</span>
                                @endif
                            </label>
                            <div class="p-4 bg-slate-50 rounded-lg border border-slate-200">
                                <p class="text-sm text-gray-800 whitespace-pre-line">{{ $facilityReport->verification_note }}</p>
                            </div>
                            @if($facilityReport->verified_at)
                                <p class="text-xs text-gray-500 mt-2">
                                    Verified on {{ $facilityReport->verified_at->format('F d, Y \a\t H:i') }}
                                </p>
                            @endif
                        </div>
                    @endif
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                        <h2 class="text-lg font-semibold text-gray-800 flex items-center">
                            ...
                            Evidence ({{ $facilityReport->evidence->count() }})
                        </h2>
                    </div>
                    <div class="p-6">
                ...
                <div class="p-6">
                    <div class="flex items-center mb-4">
                        <div class="w-16 h-16 rounded-full bg-slate-200 flex items-center justify-center text-slate-700 font-bold text-xl">
                            {{ strtoupper(substr($facilityReport->student->full_name, 0, 1)) }}
                        </div>
                        <div class="ml-4">
                            <p class="text-lg font-semibold text-gray-900">{{ $facilityReport->student->full_name }}</p>
                            <p class="text-sm text-gray-500">NIS: {{ $facilityReport->student->nis }}</p>
                        </div>
                    </div>
                    <div class="space-y-3 pt-4 border-t border-gray-100">
                        @if($facilityReport->student->email)
                            <div class="flex items-center text-sm">
                                ...
                                <span class="text-gray-700">{{ $facilityReport->student->email }}</span>
                            </div>
                        @endif
                        @if($facilityReport->student->phone_number)
                            <div class="flex items-center text-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 text-gray-400 mr-2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25Z" />
                                </svg>
                                <span class="text-gray-700">{{ $facilityReport->student->phone_number }}</span>
                            </div>
                        @endif
                        <div class="flex items-center text-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 text-gray-400 mr-2">
                            ...
                            <span class="text-gray-700">Joined {{ $facilityReport->student->created_at->format('M Y') }}</span>
                        </div>
                    </div>
                    <a href="{{ route('admin.students.show', $facilityReport->student->id) }}"
        ...
        @else
            <!-- Processed Status Card -->
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6">
                        @if($facilityReport->status === 'verified')
                            <div class="flex items-center p-4 bg-green-50 rounded-lg border border-green-200">
                                ...
                                <div class="ml-3">
                                    ...
                                    <p class="text-xs text-green-600 mt-0.5">
                                        @if($facilityReport->verifiedByAdmin)
                                            by {{ $facilityReport->verifiedByAdmin->full_name }}
                                        @endif
                                    </p>
                                </div>
                            </div>
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        ...
                        <div class="flex gap-3">
                            ...
                            <div class="flex-1 pt-1">
                                ...
                                <p class="text-xs text-gray-500">{{ $facilityReport->created_at->format('M d, Y \a\t H:i') }}</p>
                                <p class="text-xs text-gray-600 mt-1">by {{ $facilityReport->student->full_name }}</p>
                            </div>
                        </div>
                    ...
                    @if($facilityReport->verified_at)
                        <!-- Report Verified/Rejected -->
                            <div class="flex gap-3">
                                <div class="flex-shrink-0 w-8 h-8 rounded-full {{ $facilityReport->status === 'verified' ? 'bg-green-100' : 'bg-gray-100' }} flex items-center justify-center">
                                    @if($facilityReport->status === 'verified')
                                ...
                                <div class="flex-1 pt-1">
                                    <p class="text-sm font-medium text-gray-900">
                                        Report {{ ucfirst($facilityReport->status) }}
                                    </p>
                                    <p class="text-xs text-gray-500">{{ $facilityReport->verified_at->format('M d, Y \a\t H:i') }}</p>
                                    @if($facilityReport->verifiedByAdmin)
                                        <p class="text-xs text-gray-600 mt-1">by {{ $facilityReport->verifiedByAdmin->full_name }}</p>
                                    @endif
                                </div>
                            </div>
                        @endif
    ...
    <div x-data="{
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            <div x-show="open"
                ...
                <form method="POST" action="{{ route('admin.facility-reports.verify', $facilityReport->id) }}">
                    @csrf
                    @method('POST')
                    ...
                    <div class="flex items-center mb-4">
                        ...
                        <div class="ml-3">
                            ...
                            <p class="text-sm text-gray-500">Confirm this facility report is valid</p>
                        </div>
                    </div>
    ...
    <div x-data="{
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            <div x-show="open"
                ...
                <form method="POST" action="{{ route('admin.facility-reports.reject', $facilityReport->id) }}">
                    @csrf
                    @method('POST')
