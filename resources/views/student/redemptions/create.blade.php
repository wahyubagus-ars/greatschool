...
@section('title', __('Generate Redemption QR'))

@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">{{ __('Generate Redemption QR') }}</h1>
            <p class="mt-1 text-gray-600">{{ __('Redeem your points for canteen vouchers') }}</p>
        </div>
        ...
        <div class="p-5 md:p-6">
            <div class="mb-6 p-4 bg-indigo-50 rounded-xl border border-indigo-200">
                <div class="flex items-start">
                    ...
                    <div class="ml-3">
                        <p class="text-sm font-medium text-indigo-800">{{ __('How it works') }}</p>
                        <ul class="mt-1 text-sm text-indigo-700 space-y-1">
                            <li class="flex items-start">
                                ...
                                <span>{{ __('Enter the number of points you want to redeem (minimum 10 points)') }}</span>
                            </li>
                            <li class="flex items-start">
                                <span class="inline-flex items-center justify-center w-1.5 h-1.5 mr-2 mt-1 rounded-full bg-indigo-400"></span>
                                <span>{{ __('10 points = 1,000 IDR (Rp 1,000)') }}</span>
                            </li>
                            <li class="flex items-start">
                                <span class="inline-flex items-center justify-center w-1.5 h-1.5 mr-2 mt-1 rounded-full bg-indigo-400"></span>
                                <span>{{ __('QR code is valid for 15 minutes after generation') }}</span>
                            </li>
                            <li class="flex items-start">
                                <span class="inline-flex items-center justify-center w-1.5 h-1.5 mr-2 mt-1 rounded-full bg-indigo-400"></span>
                                <span>{{ __('Show QR code to canteen staff to redeem your voucher') }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <form method="POST" action="{{ route('student.redemptions.store') }}" class="space-y-6">
                ...
                <div>
                    <label for="points" class="block text-sm font-medium text-gray-700 mb-1.5">
                        {{ __('Points to Redeem') }} <span class="text-red-500">*</span>
                    </label>
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <input type="number"
                               ...
                               placeholder="{{ __('Enter points (min 10)') }}"
                               required>
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                            <span class="text-gray-500 sm:text-sm">{{ __('pts') }}</span>
                        </div>
                    </div>
                    <p class="mt-2 text-sm text-gray-500">
                        {{ __('Available points:') }} <span class="font-medium text-gray-900">{{ $student->available_points }}</span> •
                        {{ __('Equivalent to:') }} <span class="font-medium text-green-700">Rp {{ number_format(($student->available_points * 100), 0, ',', '.') }}</span>
                    </p>
                    @error('points')
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-gray-700">{{ __('Estimated Value:') }}</span>
                        <span id="idr-value" class="text-lg font-bold text-green-700">Rp 1,000</span>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">{{ __('Calculation updates as you type') }}</p>
                </div>
