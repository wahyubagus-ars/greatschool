...
@section('title', __('Submit Bullying Report'))

@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">{{ __('Submit Bullying Report') }}</h1>
            <p class="mt-1 text-gray-600">{{ __('Provide accurate information to help us address bullying incidents effectively.') }}</p>
        </div>
...
@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="border-b border-gray-200 bg-gray-50 px-5 py-4">
            <h2 class="text-lg font-medium text-gray-800">{{ __('Report Details') }}</h2>
            <p class="mt-1 text-sm text-gray-600">{{ __('All fields marked with * are required') }}</p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                ...
                <div>
                    <label for="reporter_type" class="block text-sm font-medium text-gray-700 mb-1.5">
                        {{ __('I am a') }} <span class="text-red-500">*</span>
                    </label>
                    <select name="reporter_type" id="reporter_type"
                        ...
                        <option value="victim" {{ old('reporter_type') == 'victim' ? 'selected' : '' }}>{{ __('Victim') }}</option>
                        <option value="witness" {{ old('reporter_type') == 'witness' ? 'selected' : '' }}>{{ __('Witness') }}</option>
                    </select>
                    @error('reporter_type')
                ...
                <div>
                    <label for="incident_date" class="block text-sm font-medium text-gray-700 mb-1.5">
                        {{ __('Date of Incident') }} <span class="text-red-500">*</span>
                    </label>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" id="title"
                           value="{{ old('title') }}"
                           placeholder="{{ __('Brief summary of the incident (e.g., \'Pushed in hallway during lunch\')') }}"
                           class="w-full rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 px-4 py-2.5"
                           required>
                    @error('title')
                    <p class="mt-1.5 text-sm text-red-600 flex items-center">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="location" id="location"
                           value="{{ old('location') }}"
                           placeholder="{{ __('Specific location (e.g., \'2nd floor hallway near restroom\', \'Cafeteria table #4\')') }}"
                           class="w-full rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 px-4 py-2.5"
                           required>
                    @error('location')
                    <p class="mt-1.5 text-sm text-red-600 flex items-center">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="6"
                              placeholder="{{ __('Provide detailed information about what happened. Include:\n- What exactly occurred\n- Who was involved (names if known)\n- When it happened\n- Any witnesses present\n- How it made you feel') }}"
                    ...
                    <p class="mt-2 text-xs text-gray-500">
                        {{ __('Your report is confidential. Only authorized school staff will review this information.') }}
                    </p>
                </div>
                ...
                <div class="md:col-span-2">
                    <label for="evidence" class="block text-sm font-medium text-gray-700 mb-1.5">
                        {{ __('Evidence (Photos, Videos, Documents)') }} <span class="text-gray-400">({{ __('Optional') }})</span>
                    </label>
                    <div class="mt-1 flex justify-center px-4 pt-4 pb-5 border-2 border-gray-300 border-dashed rounded-xl">
                        <div class="text-center">
                            ...
                            <p class="mt-2 text-xs text-gray-500">
                                {{ __('PNG, JPG, GIF, PDF, DOC, DOCX (max 10MB per file)') }}
                            </p>
                        </div>
                    </div>
                    @error('evidence')
        ...
        <div class="border-t border-gray-200 bg-gray-50 px-5 py-4 rounded-b-xl">
            <div class="flex items-start text-sm text-gray-600">
                ...
                <span>
                <strong class="text-gray-800">{{ __('Confidentiality Notice:') }}</strong>
                {{ __('All reports are handled with strict confidentiality. Your identity will be protected to the fullest extent possible under school policy.') }}
            </span>
            </div>
        </div>
    </div>
