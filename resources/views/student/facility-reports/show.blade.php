...
@section('title', 'Facility Report Details - ' . $facilityReport->title)

@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Facility Report Details</h1>
            <p class="mt-1 text-gray-600">Report submitted on {{ $facilityReport->created_at->format('M d, Y') }}</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="px-5 py-4 border-b border-gray-200 bg-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div class="text-sm text-gray-500 flex items-center">
                ...
                Submitted: {{ $facilityReport->created_at->format('M d, Y \a\t h:i A') }}
            </div>
        </div>
        ...
        <div class="p-5 md:p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                <div>
                    ...
                    <p class="text-base text-gray-800 font-medium">{{ $facilityReport->location }}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-500 mb-1.5">Title</h3>
                    <p class="text-lg text-gray-900 font-semibold">{{ $facilityReport->title }}</p>
                </div>
        ...
        <!-- Verification Notes -->
            @if($facilityReport->verification_note || $facilityReport->verified_at)
                <div class="pt-6 border-t border-gray-200">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 flex-1">
                            <h3 class="text-sm font-medium text-gray-500">Maintenance Response</h3>
                            <div class="mt-2 bg-gray-50 rounded-lg p-4 text-gray-800">
                                {{ $facilityReport->verification_note ?: 'No additional notes provided.' }}
                            </div>
                            @if($facilityReport->verified_at)
                                <p class="mt-2 text-xs text-gray-500 flex items-center">
                                    ...
                                    Completed on {{ $facilityReport->verified_at->format('M d, Y \a\t h:i A') }}
                                </p>
                            @endif
                        </div>
                    </div>
