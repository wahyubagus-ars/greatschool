...
@section('title', __('Quiz Results - :title', ['title' => $quiz->title]))

@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">{{ __('Quiz Results') }}</h1>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="px-5 py-6 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white text-center">
                <div class="inline-flex items-center justify-center w-24 h-24 rounded-2xl bg-white/20 backdrop-blur-sm mb-4 p-3">
                    <span class="text-3xl md:text-4xl font-bold text-white">
                        {{ $correctCount }}<span class="text-indigo-200 mx-1">/</span>{{ $result->total_questions }}
                    </span>
                </div>
                ...
                <div class="mt-3 flex items-center justify-center space-x-2">
                    ...
                    <span class="px-3 py-1 rounded-full text-sm font-medium
                        ...
                        {{ $isPassing ? __('Passed') : __('Needs Improvement') }}
                    </span>
                </div>
            </div>
            ...
            <div class="px-5 py-6 border-b border-gray-200 text-center">
                ...
                <p class="text-lg font-bold text-gray-900">{{ __('You earned :points points!', ['points' => $result->points_earned]) }}</p>
                <p class="mt-1 text-gray-600">
                    @if($correctCount > 0)
                        {{ __(':count correct answer(s) × :pts pts each', ['count' => $correctCount, 'pts' => $pointsPerQuestion]) }}
                    @else
                        {{ __('No correct answers this time. Review the materials and try again!') }}
                    @endif
                </p>
            </div>
            ...
            <div class="p-5 md:p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div class="bg-gray-50 rounded-lg p-4 text-center">
                        <p class="text-sm font-medium text-gray-500">{{ __('Correct Answers') }}</p>
                        <p class="mt-1 text-2xl font-bold text-green-600">{{ $correctCount }}</p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-4 text-center">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 text-center">
                        <p class="text-sm font-medium text-gray-500">{{ __('Points Earned') }}</p>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3 flex-1">
                            <p class="text-sm font-medium text-{{ $isPassing ? 'green' : 'amber' }}-800">
                                {{ $isPassing ? __('Great job!') : __('Keep practicing!') }}
                            </p>
                            <p class="mt-1 text-sm text-{{ $isPassing ? 'green' : 'amber' }}-700">
                                @if($isPassing)
                                    {{ __('You\'ve demonstrated strong understanding of this topic. Keep up the excellent work!') }}
                                @else
                                    {{ __('Don\'t worry—every expert was once a beginner. Review the literacy materials and try again to improve your score.') }}
                                @endif
                            </p>
                        </div>
                    </div>
        ...
        <div class="mt-6 bg-blue-50 rounded-xl p-4 border border-blue-200">
            <div class="flex">
                ...
                <div class="ml-3">
                    <p class="text-sm font-medium text-blue-800">{{ __('Points Updated') }}</p>
                    <p class="mt-1 text-sm text-blue-700">
                        {{ __('Your total points have been updated to :points in your dashboard.', ['points' => Auth::guard('student')->user()->total_points]) }}
                    </p>
                </div>
            </div>
        </div>
