...
@section('title', __('Take Quiz - :title', ['title' => $quiz->title]))
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-gray-600">
                {{ __(':current of :total questions', ['current' => $currentIndex + 1, 'total' => $totalQuestions]) }} • {{ $quiz->duration_minutes }} {{ __('minutes remaining') }}
            </p>
        </div>
        <div class="flex items-center bg-indigo-50 text-indigo-700 px-4 py-2 rounded-xl text-sm font-medium">
            ...
            <span class="font-medium">{{ $quiz->points_per_quiz ?? 10 }} {{ __('pts/question') }}</span>
        </div>
    </div>
@endsection
...
@section('content')
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('quizComponent', (initialSeconds, currentQuestionId, savedAnswers, storeAnswerUrl, csrfToken, submitConfirmMsg) => ({
    ...
    <div class="max-w-3xl mx-auto"
         x-data="quizComponent(
            ...
            '{{ csrf_token() }}',
            @json(__('Are you sure you want to submit the quiz?\n\nYou cannot change answers after submission.'))
             )"
         x-init="initTimer()">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="px-5 py-3 bg-gray-50 border-b border-gray-200">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-medium text-gray-600">
                        {{ __('Question :current of :total', ['current' => $currentIndex + 1, 'total' => $totalQuestions]) }}
                    </span>
            ...
            <div class="px-5 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div class="text-sm text-gray-600">
                    <span class="font-medium text-gray-900">{{ $currentIndex + 1 }}</span> {{ __('of') }} <span class="font-medium text-gray-900">{{ $totalQuestions }}</span> {{ __('questions') }}
                </div>
        ...
        <div class="mt-6 bg-amber-50 rounded-xl p-4 border border-amber-200">
            <div class="flex">
                ...
                <div class="ml-3 flex-1">
                    <p class="text-sm font-medium text-amber-800">{{ __('Quiz Rules') }}</p>
                    <ul class="mt-1 text-sm text-amber-700 space-y-1">
                        <li class="flex items-start">
                            ...
                            <span>{{ __('You have :minutes minute(s) to complete this quiz', ['minutes' => $quiz->duration_minutes]) }}</span>
                        </li>
                        <li class="flex items-start">
                            <span class="inline-flex items-center justify-center w-1.5 h-1.5 mr-2 mt-1 rounded-full bg-amber-400"></span>
                            <span>{{ __('Unanswered questions will be marked incorrect') }}</span>
                        </li>
                        <li class="flex items-start">
                            <span class="inline-flex items-center justify-center w-1.5 h-1.5 mr-2 mt-1 rounded-full bg-amber-400"></span>
                            <span>{{ __('Points are awarded only for correct answers (:points pts each)', ['points' => $quiz->points_per_quiz ?? 10]) }}</span>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <script>
        window.addEventListener('beforeunload', (e) => {
            if (document.querySelector('[x-data="quizComponent"]')) {
                e.preventDefault();
                e.returnValue = @json(__('You have an active quiz session. Are you sure you want to leave? Your progress will be lost.'));
            }
        });
    </script>
@endsection
