...
@section('page-header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-gray-600">{{ __('Test your knowledge and earn points for correct answers') }}</p>
        </div>
    </div>
@endsection
...
@section('content')
    @if($quizzes->isEmpty())
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-800 mb-2">{{ __('No quizzes available') }}</h3>
            <p class="text-gray-500 mb-6 max-w-md mx-auto">
                {{ __('Quizzes will be available soon. Complete literacy materials to unlock them and earn points!') }}
            </p>
        </div>
    @else
                    ...
                    <div class="p-5">
                        ...
                        <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-gray-500 mb-4 pt-3 border-t border-gray-100">
                            <div class="flex items-center">
                                ...
                                <span>{{ $quiz->questions_count ?? 0 }} {{ __('questions') }}</span>
                            </div>
                            <div class="flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-4 h-4 mr-1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <span>{{ $quiz->duration_minutes }} {{ __('min') }}</span>
                            </div>
                            <div class="flex items-center">
                                ...
                                <span>{{ $quiz->points_per_quiz ?? 10 }} {{ __('pts/question') }}</span>
                            </div>
                        </div>
